...
class CdkLambdaEc2InstanceAutoStack(Stack):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        security_groups: list[ec2.SecurityGroup] = [
            factory.ec2_security_group(
                self,
                [factory.EC2_, factory.INSTANCE_, factory.LAMBDA_],
                f"Security group for {factory.EC2_.upper()} {factory.INSTANCE_} {factory.AUTO_} {factory.START_}/{factory.STOP_} Lambda functions.",
        ...
        for i in [factory.START_, factory.STOP_]:
            ...
            role: iam.Role = factory.iam_role_lambda(
                ...
                custom_policies=[
                    iam.PolicyStatement(
                        actions=[factory.join_sep_colon([factory.EC2_, "DescribeInstances"])],
                        resources=factory.ALL_RESOURCES,
                    ),
                    iam.PolicyStatement(
                        actions=[
                            factory.join_sep_colon(
                                [factory.EC2_, ("StopInstances" if i == factory.STOP_ else "StartInstances")]
                            )
                        ],
                        resources=[factory.format_arn_custom(self, service=factory.EC2_, resource=factory.INSTANCE_)],
                    ),
                ],
            )
            ...
            factory.lambda_function(
                self,
                func_name,
                factory.get_path([factory.EC2_, func_name]),
                f"{factory.AUTO_.capitalize()} {i} {factory.EC2_.upper()} instances based on `{tag_key}` tag.",
                {
                    k: v
                    for k, v in {
                ...
                events_rules=factory.lambda_function_events_rules_cron(
                    ...
                    [([func_name], factory.schedules.window_ec2_ecs_week_days, {})],
                ),
